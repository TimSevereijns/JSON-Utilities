...
struct has_emplace_back<Type, void_t<decltype(std::declval<Type&>().emplace_back(
...
template <typename Type>
struct has_emplace<Type, void_t<decltype(std::declval<Type&>().emplace(
   std::declval<typename Type::value_type&&>()))>> : std::true_type
{
};
...
struct treat_as_array<
    Type, void_t<typename Type::value_type, typename Type::reference,
                 typename Type::const_reference, typename Type::iterator,
...
template <typename Writer, typename CharacterType, typename CharacterTraits, typename Allocator>
...
namespace serializer {
inline std::string to_narrow_json_key(const std::string& data) noexcept { return data; }

inline std::wstring to_wide_json_key(const std::wstring& data) noexcept { return data; }
...
namespace serializer {
template <typename Writer> void to_json(Writer& writer, bool data) { writer.Bool(data); }

template <typename Writer> void to_json(Writer& writer, std::int32_t data) { writer.Int(data); }

template <typename Writer> void to_json(Writer& writer, std::uint32_t data) { writer.Uint(data); }

template <typename Writer> void to_json(Writer& writer, std::int64_t data) { writer.Int64(data); }

template <typename Writer> void to_json(Writer& writer, std::uint64_t data) { writer.Uint64(data); }
             ...
             const std::basic_string<CharacterType, CharacterTraits, Allocator>& data)
{
  static_assert(
      std::is_same<
          typename std::basic_string<CharacterType, CharacterTraits, Allocator>::value_type,
...
{
  writer.StartArray();
  for (const auto& item : container) { to_json(writer, item); }
  writer.EndArray();
}
...
{
  static_assert(
      std::disjunction<std::is_same<DataType, typename ContainerType::value_type>,
...
auto dispatch_insertion(const rapidjson::Value& value, ContainerType& container) ->
    typename std::enable_if<std::is_same<int, typename ContainerType::value_type>::value>::type
{
  if (value.IsInt()) {
...
template <typename InsertionPolicy, typename ContainerType>
auto dispatch_insertion(const rapidjson::Value& value, ContainerType& container) ->
    typename std::enable_if<std::is_same<double, typename ContainerType::value_type>::value>::type
{
  if (value.IsDouble()) {
...
template <typename InsertionPolicy, typename ContainerType>
auto dispatch_insertion(const rapidjson::Value& value, ContainerType& container) ->
    typename std::enable_if<traits::treat_as_array<typename ContainerType::value_type>::value>::type
{
  using nested_container_type = typename ContainerType::value_type;

  if (value.IsArray()) {
    auto nestedContainer = from_json_array<InsertionPolicy, nested_container_type>(value.GetArray());
...
auto to_key_value_pair(const rapidjson::GenericMember<EncodingType, AllocatorType>& member) ->
    typename std::enable_if<std::is_same<typename ContainerType::second_type, int>::value,
...
auto to_key_value_pair(const rapidjson::GenericMember<EncodingType, AllocatorType>& member) ->
    typename std::enable_if<std::is_same<typename PairType::second_type, std::uint32_t>::value,
                            std::pair<std::string, std::uint32_t>>::type
{
  return std::make_pair<std::string, std::uint32_t>(member.name.GetString(),
                                                    member.value.GetUint());
}

template <typename PairType, typename EncodingType, typename AllocatorType>
auto to_key_value_pair(const rapidjson::GenericMember<EncodingType, AllocatorType>& member) ->
    typename std::enable_if<std::is_same<typename PairType::second_type, std::int64_t>::value,
                            std::pair<std::string, std::int64_t>>::type
{
  return std::make_pair<std::string, std::int64_t>(member.name.GetString(),
                                                   member.value.GetInt64());
}

template <typename PairType, typename EncodingType, typename AllocatorType>
auto to_key_value_pair(const rapidjson::GenericMember<EncodingType, AllocatorType>& member) ->
    typename std::enable_if<std::is_same<typename PairType::second_type, std::uint64_t>::value,
                            std::pair<std::string, std::uint64_t>>::type
{
  return std::make_pair<std::string, std::uint64_t>(member.name.GetString(),
                                                    member.value.GetUint64());
}

template <typename PairType, typename EncodingType, typename AllocatorType>
auto to_key_value_pair(const rapidjson::GenericMember<EncodingType, AllocatorType>& member) ->
    typename std::enable_if<std::is_same<typename PairType::second_type, double>::value,
                            std::pair<std::string, double>>::type
{
  return std::make_pair<std::string, double>(member.name.GetString(), member.value.GetDouble());
}

template <typename InsertionPolicy, typename EncodingType, typename AllocatorType,
          typename ContainerType>
auto dispatch_insertion(const rapidjson::GenericMember<EncodingType, AllocatorType>& member,
                        ContainerType& container)
{
...
auto from_json(const rapidjson::Document& document) ->
    typename std::enable_if<std::conjunction<traits::has_emplace_back<ContainerType>,
...
template <typename ContainerType>
auto from_json(const rapidjson::Document& document) ->
    typename std::enable_if<std::conjunction<traits::has_emplace<ContainerType>,
                                             traits::treat_as_object<ContainerType>>::value,
                            ContainerType>::type
{
  ContainerType container{};
